...
var Menu2 = {//판매자 메뉴
	...
	methods: {
		edit:function(num){
			//alert(num)
			app.$router.push({name:'EditProduct', params:{num:num}});//path로 params 가져가면 안됨.
		},
		del:function(num){
...
var EditProduct = {
	...
	methods:{
		...
		const form = new FormData();//폼파람 객체.
			...
			axios.put('http://localhost:8081/products/', form, {headers:{'Content-Type':'multipart/form-data'}})
				.then(function(response) {//response: 응답객체
					//alert(response.status);
					if (response.data.result) {
						alert('편집완료')
